...
<body class="overflow-x-hidden min-w-[1440px] font-sans antialiased bg-slate-50 text-slate-900">
    <div class="min-w-[1440px] flex">

        <aside class="w-72 min-w-72 bg-[#001a4d] text-white hidden md:flex flex-col sticky top-0 self-start h-screen z-20">
            {{-- Brand Logo Section --}}
            ...
            <nav class="flex-grow px-4 space-y-1.5 overflow-y-auto scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
                ...
                <x-nav-link-item :href="route('business-requests.index')" :active="request()->is('business-requests') || request()->routeIs('business-requests.index')" icon="shield-check" label="依頼承認・管理" />

                <x-nav-link-item :href="route('business-requests.my_tasks')" :active="request()->routeIs('business-requests.my_tasks')" icon="check-square" label="担当作業一覧" />
                @endif
            </nav>
    ...
    {{-- Notification Toast --}}
    @if(session('success'))
    <div id="success-toast" class="fixed top-5 right-5 z-[100] transform transition-all duration-500 ease-in-out">
